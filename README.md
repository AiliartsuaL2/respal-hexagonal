...
- 회원
  - 토큰이 필요한 경우
    ...
    - 회원 정보를 수정 할 수 있다.
  - 토큰이 필요 없는 경우
    - 일반 로그인을 통해 Token을 발급 받을 수 있어야 한다.
      - email, 비밀번호로 로그인을 한다.
    - OAuth 로그인을 통해 Token을 발급 받을 수 있어야 한다.
    - 이력서 회원 멘션 시 이메일로 회원을 검색 할 수 있다.
- 인증
  - 인증 관련 (Jwt Token) 기능 처리   
  - 회원 엔티티와 의존성을 제거하기 위해 UserAccount라는 테이블에 추가 저장
    - 회원가입시 해당 테이블에 회원 ID 저장 
- 이력서
  - 토큰이 필요한 경우
    - 이력서를 생성 할 수 있다.
    - 본인의 이력서를 수정 할 수 있다.
...
- 댓글
  - 토큰이 필요한 경우
    - 실시간으로 해당 이력서에대한 댓글의 현황을 조회 할 수 있다.
  - 토큰이 필요 없는 경우
    - 본인이 작성하거나 본인이 태그당한 글에  
    - 본인이 작성한 이력서의 댓글이거나, 본인 댓글을 삭제 할 수 있다.
